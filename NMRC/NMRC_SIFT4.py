...
def construct_neighbor(m1, K, index_N, I0):
    """
    计算m1[I0]在m1[index_N]中构成的邻域
    :param m1: SIFT给出的初始匹配点对
    :param K: 邻域点数
    :param index_N: 用于构成邻域的点集在m1中的索引
    :param I0: 计算邻域的目标点集在m1中的索引
    :return: index_Um: m1[I0]的邻域点集在m1中的索引
    """
    s_m1 = I0.shape[0]
    s_Nm = index_N.shape[0]
    Dis_N = np.array(np.zeros((s_m1, s_Nm)), float)  # 点之间的距离
    for i in range(s_m1):
        for j in range(s_Nm):
            if I0[i] == index_N[j]:
                Dis_N[i, j] = 1e9
            else:
                Dis_N[i, j] = ((m1[I0[i], 0] - m1[index_N[j], 0])**2 + (m1[I0[i], 1] - m1[index_N[j], 1])**2)**(1 / 2)
                if Dis_N[i, j] == 0:
                    Dis_N[i, j] = 1e9
    # 前K个最小的索引,即邻域点的索引
    index_K = index_N[np.argpartition(Dis_N, K)[:, 0:K]]
    return index_K
...
def interation_neighbor(m1, m2, M, K, k, nm):
    ...
    for m in range(M):
        index_X = construct_neighbor(m1, K, index_N, index_std)  # x的邻域
        index_Y = construct_neighbor(m2, K, index_N, index_std)  # y的邻域
        ...
        for i in range(N):
            ...
            if ratio > nm[m]:
                ti = ti + 1
                index_Um[ti] = index_std[i]  # 存放筛选出来的匹配点对的索引
        if ti > K:
...
def same_neighbor(index_X, index_Y, k, I0, yita):
    """
    ...
    for i in range(N):
        ...
        if ratio >= yita:
            ti = ti + 1
            index_Um[ti] = I0[index_std[i]]
            index_I0 = index_Um[0:ti]

    return index_I0


def calculate_dist(m1, m2, K, index, I0, option, yita):
    """
    计算m1[I0]与邻域m1[index]的距离
    :param m1:
    :param m2:
    :param K:
    :param index: 用于构造邻域的点集在m1中的索引 m1[index]
    :param I0: m1[I0]
    :return:
    """
    index_I0 = None
    index_X = construct_neighbor(m1, K, index, I0)  # x的邻域
    index_Y = construct_neighbor(m2, K, index, I0)  # y的邻域
    if option == 1:
        index_I0 = same_neighbor(index_X, index_Y, K, I0, yita)  # K=k
        N = index_I0.shape[0]
...
def NMRC(m1, m2, lamda, K, M, nm, k, yita):
    ...
    dist, index_X, index_Y, index_I0 = calculate_dist(m1, m2, K, index, index_std, 1, yita)
    ...
    # 用I0计算距离
    if K <= N:
        dist, index_X, index_Y, index_I0 = calculate_dist(m1, m2, K, index, I0, 0, yita)  # index_X 是用I0构建邻域的索引
    else:
        print("I0构成的邻域过小,K近邻大小为", N - 1)
        dist, index_X, index_Y, index_I0 = calculate_dist(m1, m2, N - 1, index, I0, 0, yita)  # index_X 是用I0构建邻域的索引
